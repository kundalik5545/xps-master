...
import {
  addUpdateEmSqlScript,
  deleteEmSqlScript,
} from "@/actions/emember/em_SqlScripts";
...
import EmScriptForm from "./EmScriptForm";

const EmScripts = ({ scriptsData, menuId }) => {
  ...
  useEffect(() => {
    setScripts(scriptsData?.emScripts);
  }, [scriptsData]);

  const handleCopy = async (emSqlScript, id) => {
    try {
      await navigator.clipboard.writeText(emSqlScript || "");
  ...
  const { handleFormSubmit: handleSubmit, loading: loadingSubmit } =
    useFormSubmit({
      formSubmitAction: addUpdateEmSqlScript,
      isEditing: isEditing,
  ...
  const { onDelete, loading: deleting } = useSingleDelete({
    deleteAction: deleteEmSqlScript,
    setData: setScripts,
  });
              ...
              >
                <CardHeader className="flex items-start justify-between gap-2">
                  ...
                  <CardAction className="flex items-center gap-2 shrink-0">
                    ...
                    <Button
                      onClick={() => handleCopy(script.emSqlScript, script.id)}
                      size="icon"
                      variant="secondary"
                      title="Copy Em SQL"
                    >
                ...
                <CardContent className="space-y-3">
                  <div className="rounded-md border bg-muted/40">
                    ...
                    >
                      {script.emSqlScript || "-- No SQL provided"}
                    </SyntaxHighlighter>
                  </div>
                  {script.emScriptInfo && (
